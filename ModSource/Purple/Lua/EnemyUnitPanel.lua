-- EnemyUnitPanel
-- Author: LynxStar
-- DateCreated: 9/7/2014 10:31:04 PM
--------------------------------------------------------------
-------------------------------------------------
							...
							for i = 0, iNumUnits do
								...
								if (pUnit ~= nil and not pUnit:IsInvisible(iTeam, false)) then
									...
									--if (pUnit:GetDomainType() ~= DomainTypes.DOMAIN_AIR) then
										...
										local canalOverride = pPlot:HasCanal() and pUnit:GetDomainType() == DomainTypes["DOMAIN_SEA"] and pHeadUnit:GetDomainType() == DomainTypes["DOMAIN_LAND"]

										if (pHeadUnit:GetDomainType() == pUnit:GetDomainType() or pHeadUnit:IsRanged() or canalOverride) then
